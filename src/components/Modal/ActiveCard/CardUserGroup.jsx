...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lấy activeBoard từ redux ra để mục đích lấy được toàn bộ thông tin những thành viên của cái board thông qua fiel: FE_allUsers
  ...
  const handleUpdateCardMemeber = (user) => {
    // console.log(user)
    // Tạo một biến incomingMemberInfo để gửi cho BE, với thông tin chính là userId và action là xóa khỏi card (REMOVE) hoặc thêm vào card (ADD)
          ...
          {board.FE_allUsers.map((user, index) =>
            <Tooltip title={user.displayName} key={index}>
              ...
              <Badge
                ...
                onClick={() => handleUpdateCardMemeber(user)}
              >
                <Avatar
